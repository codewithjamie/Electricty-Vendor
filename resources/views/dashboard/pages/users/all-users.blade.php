@section('page_title', 'BuyPowerNow - All Users')
                            ...
                            @if(!empty($users) && $users->count())
                                @foreach($users as $user)
                                    <tr class="btn-reveal-trigger">
                                        ...
                                        <td class="align-middle">{{$user->login_time ?? "not stamped"}}</td>
                                            ...
                                            <div class="dropdown text-sans-serif"><button class="btn btn-link text-600 btn-sm dropdown-toggle btn-reveal mr-3" type="button" id="dropdown0" data-toggle="dropdown" data-boundary="html" aria-haspopup="true" aria-expanded="false"><span class="fas fa-ellipsis-h fs--1"></span></button>
                                                <div class="dropdown-menu dropdown-menu-right border py-0" aria-labelledby="dropdown0">
                                                    <div class="bg-white py-2">
                                                        ...
                                                        <a class="dropdown-item" href="/dashboard/users/edit-user/{!! $user->id !!}">Edit</a>
                                                        <a class="dropdown-item" href="/dashboard/users/fund-wallet/{!! $user->id !!}">Fund Wallet</a>
                                                        <div class="dropdown-divider"></div>
                                                        <a class="dropdown-item text-danger" href="/dashboard/users/delete-user/{!! $user->id !!}">Delete</a>
                                                    </div>
                                                </div>
                                            </div>
                                        </td>
